...
def index():
    ...
    stock_api = current_app.config['STOCK_API_KEY']
    news_api = current_app.config['NEWS_API_KEY']
    ...
    if request.method == 'POST':
        try:
            ...
            for article in news_result:
                if news_count >= 5:
                    break
                if article['title'] != '' and article['url'] != '' and article['urlToImage'] != '' and article['publishedAt'] != '':
            ...
            return render_template('index.html', outlook={}, summary={'change': 0.0, 'changePercent': 0.0}, news=[], error='Yes')

    return render_template('index.html', outlook=outlook_result, summary=summary_result, news=top_five_news, error='No')


@main.route('/stock', methods=['POST', 'GET'])
def stock_chart():
    global stock_name
    stock_api = current_app.config['STOCK_API_KEY']
    ...
    return {'charts_result': charts_result, 'chart_title': chart_title, 'chart_stock': stock_name}
